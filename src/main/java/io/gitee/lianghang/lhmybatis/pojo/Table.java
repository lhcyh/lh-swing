...
import io.gitee.lianghang.lhmybatis.utils.Utils;
...
//    private List<Table> associatedList;
    ...
    private Field getPrimaryKey(){
        for(Field fieldItem:fieldList){
            System.out.println(fieldItem.getIsPrimaryKey());
...
//        }
    ...
    public String getEntityCode(Project project){
        ...
        if(foreignKeyList.size()==0&&associatedList.size()==0){
            return null;
        }
    ...
    private String getInsertXml(String packageName,String fill){
        ...
        String xml=" parameterType=\""+packageName+".pojo."+upName+"\">\n";
    ...
    private String getUpdateXml(String packageName,String fill){
        ...
        String pKeyName=getPrimaryKey().getName();
        String pDoName= Utils.underscoreToCamel(pKeyName,false);
        String xml=fill+"<update id=\"update"+upName+"\" parameterType=\""+packageName+".pojo."+upName+"\">\n";
    ...
    private String getResultXml(Project project){
        ...
        if(entity==null){
            result=" resultType=\""+project.getBasePackage()+".pojo."+upName+"\" ";
        }
        return result;
    }
...
//
    private String getBaseMapXml(String fill,Project project){
        ...
        String xml="";
        xml+=fill+"<resultMap id=\"baseMap\" type=\""+project.getBasePackage()+".entity."+upName+"Entity\">\n";
...
//                }
                ...
                xml+=fill+"    <association fetchType=\"lazy\" property=\""+rtDoName+"\" column=\""+foreignKeyItem.getFieldName()+"\" select=\""+project.getBasePackage()+".mapper."+rtUpName+ "Mapper."+"get"+rtUpName+"By"+rtpUpName+"\"/>\n";
            }
            ...
            switch (associateForeignKey.getAssociate()){
                case OneToOneR:
                    xml+=fill+"    <association fetchType=\"lazy\" property=\""+tDoName+"\" column=\""+associateForeignKey.getReferencedFieldName()+
                            "\" select=\""+project.getBasePackage()+".mapper."+tUpName+"Mapper.get"+tUpName+"By"+tfUpName+"\"/>\n";
                    break;
                case ManyToOne:
                    xml+=fill+"    <collection fetchType=\"lazy\" property=\""+tDoName+"List\" column=\""+associateForeignKey.getReferencedFieldName()+
                            "\" select=\""+project.getBasePackage()+".mapper."+tUpName+"Mapper.get"+tUpName+"ListBy"+tfUpName+"\"/>\n";
                    break;
            }
        }
        xml+=fill+"</resultMap>\n";
        ...
        for(ForeignKey foreignKeyItem:foreignKeyList){
            if(foreignKeyItem.getAssociate()== ForeignKey.Associate.OneToOneL){
                ...
                if(rTable.getLeftJoinXml(fill,project)!=null){
                    leftJoin+=fill+"    <include refid=\""+project.getBasePackage()+".mapper."+ftUpName+"Mapper.leftJoin\"></include>\n";
                }
            }
        }
        List<Table> associatedList=project.getAssociatedListByTableName(name);
    ...
    private String getTableFieldMapping(String fill,Project project){
        ...
        for(ForeignKey foreignKeyItem:foreignKeyList){
            if(foreignKeyItem.getAssociate()== ForeignKey.Associate.OneToOneL){
                ...
                body+=fill+"            <include refid=\""+project.getBasePackage()+".mapper."+ftUpName+"Mapper.tableFieldMapping\"></include>\n";
                body+=fill+"        </when>\n";
            }
        }
        List<Table> associatedList=project.getAssociatedListByTableName(name);
    ...
    private String getGetListByExampleXml(Project project,String fill){
        String upName= Utils.underscoreToCamel(name,true);
        String xml=fill+"<select id=\"get"+upName+"ListByExample\" parameterType=\""+project.getBasePackage()+".example.Example\" "+getResultXml(project)+">\n";
    ...
    public String getXml(Project project){
        ...
        xml+="<mapper namespace=\""+ project.getBasePackage()+ ".mapper."+ upName+ "Mapper\">\n";
        xml+=getInsertXml(project.getBasePackage(),"    ")+"\n";
        xml+=getDeleteXml("    ")+"\n";
        xml+=getUpdateXml(project.getBasePackage(),"    ")+"\n";
